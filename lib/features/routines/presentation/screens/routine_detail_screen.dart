...
/// One rendered row in the exercise list: either a day-section header (for
/// multi-day imported programs) or an exercise block. Kept as a sealed type
/// instead of two parallel index spaces, since "day header at position N"
/// and "exercise at flat index M" must never be conflated.
sealed class _DetailRow {}

class _DayHeaderRow extends _DetailRow {
  final RoutineDay day;
  final int exerciseCount;
  _DayHeaderRow(this.day, this.exerciseCount);
}

class _ExerciseDetailRow extends _DetailRow {
  final HydratedRoutineExercise exercise;

  /// Index into the flat, day-ordered [HydratedRoutineDetail.exercises] list.
  /// heroEnabledList / lastSetsMap / isLast all key off this flat position,
  /// not the row's position in the rendered list.
  final int flatIndex;
  _ExerciseDetailRow(this.exercise, this.flatIndex);
}

/// Builds the rendered row list for a routine's exercise section. Single-day
/// routines (the overwhelming common case) render exactly as before: no
/// headers, one row per exercise. Multi-day programs get one header per
/// non-empty day, so a single flat SliverList still reads as a day-grouped
/// program without a nested-scroll structure.
List<_DetailRow> _buildDetailRows(HydratedRoutineDetail routine) {
  final rows = <_DetailRow>[];
  if (!routine.isMultiDay) {
    for (var i = 0; i < routine.exercises.length; i++) {
      rows.add(_ExerciseDetailRow(routine.exercises[i], i));
    }
    return rows;
  }
  var flatIndex = 0;
  for (final group in routine.exercisesByDay) {
    if (group.exercises.isEmpty) continue;
    rows.add(_DayHeaderRow(group.day, group.exercises.length));
    for (final he in group.exercises) {
      rows.add(_ExerciseDetailRow(he, flatIndex));
      flatIndex++;
    }
  }
  return rows;
...
class _RoutineDetailScreenState extends ConsumerState<RoutineDetailScreen> {
  ...
  void _startRoutine(HydratedRoutineDetail routine) {
    ...
    // Multi-day imported programs (e.g. a 4-day Upper/Lower split) can't
    // start "the routine" as one session -- ask which day first. Classic
    // single-day routines skip straight to the workout, unchanged.
    if (routine.isMultiDay) {
      _showDayPicker(routine);
      return;
    }
    _beginWorkout(routine, routine.exercises);
  }

  void _showDayPicker(HydratedRoutineDetail routine) {
    final surface = context.surface;
    HapticFeedback.selectionClick();
    showActionBottomSheet(
      context: context,
      title: 'Start which day?',
      items: [
        for (final group in routine.exercisesByDay)
  ...
  void _beginWorkout(
    ...
    List<HydratedRoutineExercise> exercises,
  ) {
    ...
    ref.read(activeWorkoutProvider.notifier).startWorkout(
          ...
          initialExercises: seedExercisesFromRoutine(
  ...
  Widget _buildLoaded(HydratedRoutineDetail routine) {
    ...
    final detailRows = _buildDetailRows(routine);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      routine.isMultiDay
                          ? '${routine.days.length}-day split · $exerciseCount exercise${exerciseCount != 1 ? 's' : ''}'
              ...
              sliver: SliverList(
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    ...
                    final exerciseRow = row as _ExerciseDetailRow;
...
class _DayHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final surface = context.surface;
    return Padding(
      padding: EdgeInsets.fromLTRB(4, isFirst ? 4 : 28, 4, 12),
      child: Semantics(
        header: true,
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.center,
