...
## Phase 2: SNACKBAR-1 — Unify Snackbar Rendering Under showAppSnackBar
...
## Phase 2: SILENT-FAILURE-1 — Surface Cloud Sign-Out & Local-Delete Failures
...
- `lib/features/auth/data/auth_repository.dart`: `signOut()` no longer
  swallows Supabase errors — callers decide whether to surface or proceed.
...
- Failure snackbars added: home_screen + workout_detail_screen (delete
  failed → "Couldn't delete that workout. Try again."), routine_detail_screen
...
cloud → `cloudSignOutFailed`, clean cloud → `complete` + sign-out called,
no user → `complete` without touching cloud).
