...
### Problem
Three compounding defects made the Exercise Library â†’ Exercise Detail Hero transition visually broken (coverâ†’contain jump on landing, spinner flash at handoff, Hero clipped by FadeTransition).
...
- **`build()` restructured**: `gifSection` (Hero wrapping `_buildGifContent`) is first child of `ListView` directly â€” completely outside `_entryFade`. Only text content below fades in
- **Reduced-motion path**: `disableAnimations=true` â†’ `ExerciseGifWidget(animate: false, fit: BoxFit.cover)`, no Hero, no AnimatedSwitcher
...
### Gate Verification Result
- [x] Format: **PASS**
- [x] Static Analysis: **PASS** (`flutter analyze --fatal-infos --fatal-warnings` â€” No issues found)
- [x] Custom Linter: **PASS**
- [x] Tests Suite: **PASS** (164/164 tests passed â€” all Hero transition tests and detail polish tests unchanged)
