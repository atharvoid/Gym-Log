...
| **A11Y-TRIPLE-ANNOUNCE** | Systemic | Wrapper `Semantics(label:)` over children publishing their own text/button nodes announced the same content 2–3× to screen readers. Fixed ProLockPill, `_PackageRow`, `_ErrorAction`, `_RecoveryAction`, MuscleSummaryStrip, `_SignOutButton`, TimeRangeFilter, sign-in button, `_EditorToolButton`, chart data-table toggle with the checklist §2 full form (`container`+`excludeSemantics`+`onTap`) | `done` | (pending) |
