...
- **`applicationId` matches OAuth Client ID:** `YES` (`applicationId` is `com.drifs.gymlog` in `android/app/build.gradle.kts`, matching the package name configuration).
...
- **Google login fix:**
  ...
  3. Find the **OAuth 2.0 Client IDs** and edit the **Android Client ID** matching package `com.drifs.gymlog`.
